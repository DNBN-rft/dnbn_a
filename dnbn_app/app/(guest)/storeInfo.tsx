...
export default function StoreInfo() {
  ...
  const fetchMoreProducts = useCallback(async () => {
    ...
    try {
      ...
      const response = await apiGet(
        `/guest/storeinfo/products?storeCode=${storeCode}&productType=${PRODUCT_TYPE_MAP[activeProductTab]}&page=${nextPage}`,
      );
  ...
  const fetchReviews = useCallback(async () => {
    ...
    try {
      ...
      const response = await apiGet(
        `/guest/storeinfo/reviews?storeCode=${storeCode}&page=0`,
      );
  ...
  const fetchMoreReviews = useCallback(async () => {
    ...
    try {
      ...
      const response = await apiGet(
        `/guest/storeinfo/reviews?storeCode=${storeCode}&page=${nextPage}`,
      );
      if (!response.ok) throw new Error("리뷰 목록을 불러오는데 실패했습니다.");
  ...
  const handleProductTabChange = useCallback(
    async (tab: ProductTabType) => {
      ...
      try {
        setLoadingProducts(true);
        const response = await apiGet(
          `/guest/storeinfo/products?storeCode=${storeCode}&productType=${PRODUCT_TYPE_MAP[tab]}&page=0`,
        );
        if (!response.ok)
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.leftSection}>
          <TouchableOpacity
            ...
            onPress={() => showLoginRequired ? setShowLoginRequired(false) : router.back()}
          >
      ...
      {showLoginRequired ? (
        <View style={loginStyles.container}>
          ...
          <TouchableOpacity style={loginStyles.backButton} onPress={() => setShowLoginRequired(false)}>
