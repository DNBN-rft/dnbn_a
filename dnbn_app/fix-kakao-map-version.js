...
try {
  ...
  console.error('[fix-kakao-map-version] Failed to parse @react-native-kakao/core/package.json:', e.message);
  process.exit(1);
}

if (!pkg.sdkVersions || !pkg.sdkVersions.android) {
  console.error('[fix-kakao-map-version] Unexpected package.json structure - sdkVersions.android not found');
  process.exit(1);
}

if (pkg.sdkVersions.android.map) {
  console.log('[fix-kakao-map-version] Already has map version:', pkg.sdkVersions.android.map);
} else {
  pkg.sdkVersions.android.map = '2.13.0';
  fs.writeFileSync(pkgPath, JSON.stringify(pkg, null, 2), 'utf8');
  console.log('[fix-kakao-map-version] SUCCESS: Added Kakao Maps Open SDK version 2.13.0');
}
